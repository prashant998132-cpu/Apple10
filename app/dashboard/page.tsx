...
export default function DashboardPage() {
  ...
  async function fetchGold() {
    ...
    try {
      ...
      if (d.gold24k && d.gold24k > 5000) {
        setGold(d);
      } else {
        setGoldErr(d.error || 'Live data nahi mili');
      }
    } catch (e: any) {
      setGoldErr('Network error');
    }
  }
    ...
    try {
      ...
      if (r.ok) {
        ...
        if (d.articles?.length > 0) {
          setNews(d.articles.slice(0,5).map((a: any) => ({ title: a.title||'', source: a.domain||'News', url: a.url||'#' })));
          return;
        }
      }
    } catch {}
    ...
    <div style={{ background: '#070810', minHeight: '100dvh', color: '#e2e8f0', fontFamily: 'sans-serif', paddingBottom: 70 }}>
      <div style={{ background: '#0a0c14', borderBottom: '1px solid rgba(255,255,255,0.06)', padding: '12px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          ...
          <div style={{ fontSize: 10, color: '#374151' }}>{time.toLocaleTimeString('hi-IN')} · {time.toLocaleDateString('hi-IN', { weekday: 'short', day: 'numeric', month: 'short' })}{updated && ' · ' + updated}</div>
        </div>
      ...
      <div style={{ padding: 14 }}>
        ...
        <div style={{ background: '#0d1117', border: '1px solid rgba(255,215,0,0.15)', borderRadius: 12, padding: 14, marginBottom: 12 }}>
          <div style={{ fontSize: 10, fontWeight: 800, color: '#fbbf24', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 10 }}>
            ...
            {gold?.source && <span style={{ fontWeight: 400, marginLeft: 6, color: '#374151', textTransform: 'none' }}>via {gold.source}</span>}
          </div>
          {goldErr ? (
            <div style={{ color: '#f87171', fontSize: 11, padding: '8px 0', display: 'flex', alignItems: 'center', gap: 8 }}>
              ❌ {goldErr}
              <button onClick={fetchGold} style={{ background: 'none', border: '1px solid #f87171', borderRadius: 6, color: '#f87171', padding: '3px 8px', cursor: 'pointer', fontSize: 10 }}>Retry</button>
            </div>
          ) : gold ? (
            <div style={{ display: 'flex', gap: 8 }}>
              {[
                ...
                <div key={item.label} style={{ flex: 1, background: 'rgba(255,215,0,0.04)', border: '1px solid rgba(255,215,0,0.1)', borderRadius: 10, padding: '10px 6px', textAlign: 'center' }}>
                  ...
                  <div style={{ fontSize: 13, fontWeight: 900, color: '#fbbf24', marginTop: 4 }}>{item.value > 0 ? '₹'+fmt(item.value) : '--'}</div>
                  ...
                  {gold.change > 0 && (
                    <div style={{ marginTop: 3, fontSize: 11, color: gold.changeDir === 'up' ? '#22c55e' : '#f87171' }}>
                      {gold.changeDir === 'up' ? '↑' : '↓'} {gold.change.toFixed(2)}%
                    </div>
                  )}
                </div>
              ))}
            ...
            <div style={{ color: '#374151', fontSize: 12, textAlign: 'center', padding: 16 }}>⏳ Live rate aa raha hai...</div>
          )}
        </div>
        ...
        <div style={{ background: '#0d1117', border: '1px solid rgba(34,197,94,0.1)', borderRadius: 12, padding: 14 }}>
          ...
          )) : <div style={{ color: '#374151', fontSize: 12, textAlign: 'center', padding: 12 }}>{loading ? '⏳ News aa rahi hai...' : 'Unavailable'}</div>}
        </div>
