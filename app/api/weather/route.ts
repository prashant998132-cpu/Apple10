...
export async function GET(req: NextRequest) {
  ...
  try {
    const r = await fetch(
      `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&current=temperature_2m,weathercode,relative_humidity_2m,windspeed_10m,apparent_temperature&daily=temperature_2m_max,temperature_2m_min,weathercode,precipitation_sum,uv_index_max&forecast_days=${days}&timezone=auto`,
    ...
    const WMO: Record<number, [string, string]> = {
      ...
      51: ['🌦️', 'Drizzle'], 53: ['🌦️', 'Drizzle'], 61: ['🌧️', 'Rain'],
      63: ['🌧️', 'Heavy rain'], 65: ['🌧️', 'Very heavy rain'], 71: ['❄️', 'Snow'],
      73: ['❄️', 'Heavy snow'], 80: ['🌦️', 'Showers'], 81: ['🌦️', 'Heavy showers'],
    ...
    return NextResponse.json({
      ...
      forecast: d.daily.time.slice(0, parseInt(days)).map((t: string, i: number) => ({
        date: t,
