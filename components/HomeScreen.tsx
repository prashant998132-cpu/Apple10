...
export default function HomeScreen({ name, onSend }: Props) {
  ...
  const [weather, setWeather] = useState<{ temp: number; icon: string; desc: string; city: string } | null>(null);
  ...
  useEffect(() => {
    ...
    try {
      const profile = JSON.parse(localStorage.getItem('jarvis_profile') || localStorage.getItem('jarvis-db-profile') || '{}');
    ...
    try {
      ...
      const diff = neetDate.getTime() - Date.now();
      if (diff > 0) setNeetDays(Math.ceil(diff / 86400000));
    } catch {}

    // Weather — dynamic location
    const loc = (() => { try { return JSON.parse(localStorage.getItem('jarvis_location') || '{}'); } catch { return {}; } })();
    const weatherLat = loc.lat || 24.53;
    const weatherLon = loc.lon || 81.3;
    const weatherCity = loc.city || name?.split(' ')[0] || 'Shehar';
    fetch(`https://api.open-meteo.com/v1/forecast?latitude=${weatherLat}&longitude=${weatherLon}&current=temperature_2m,weathercode&timezone=auto`)
      .then(r => r.json())
      .then(d => {
        const c = d.current;
        ...
        setWeather({ temp: Math.round(c.temperature_2m), icon, desc, city: weatherCity });
      }).catch(() => {});
  }, []);
          ...
          {weather ? (
            <>
              ...
              <div>
                ...
                <div style={{ fontSize: 10, color: '#475569' }}>{weather?.city || 'Aapka Shehar'}</div>
              </div>
            </>
