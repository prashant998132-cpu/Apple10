...
interface WeatherData {
  ...
  current: {
    ...
    wind: number; icon: string; desc: string; code: number;
  };
  forecast: Array<{
    date: string; max: number; min: number;
    rain: number; uv: number; icon: string; desc: string;
  }>;
}
...
export default function WeatherWidget({ compact = false }: { compact?: boolean }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/weather?lat=${lat}&lon=${lon}&city=${encodeURIComponent(city)}&days=5`)
      .then(r => r.json())
      .then(d => { if (d.current) setData(d); else setError('Data nahi mili'); })
      .catch(() => setError('Weather load fail'))
      .finally(() => setLoading(false));
  }, []);

  if (loading) return (
    <div style={{ padding: '12px 16px', background: 'rgba(15,23,42,0.9)', borderRadius: 14, color: '#374151', fontSize: 12 }}>
      ☁️ Mausam load ho raha hai...
    </div>
  );

  if (error || !data) return (
    <div style={{ padding: '12px 16px', background: 'rgba(15,23,42,0.9)', borderRadius: 14, color: '#f87171', fontSize: 12 }}>
      ⚠️ {error || 'Unavailable'}
    </div>
  );
  ...
  if (compact) return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 14px', background: 'linear-gradient(135deg,rgba(30,58,95,0.9),rgba(15,32,64,0.9))', borderRadius: 12, border: '1px solid rgba(96,165,250,0.2)' }}>
      ...
      <div>
        <div style={{ fontSize: 18, fontWeight: 900, color: '#fff' }}>{c.temp}°C</div>
  ...
  return (
    <div style={{ background: 'linear-gradient(135deg,rgba(15,23,50,0.95),rgba(10,15,35,0.95))', border: '1px solid rgba(96,165,250,0.2)', borderRadius: 16, padding: '16px', overflow: 'hidden' }}>
      {/* Current */}
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 14 }}>
        <div>
          <div style={{ fontSize: 10, color: '#60a5fa', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 4 }}>📍 {data.city}</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 4 }}>
        {data.forecast.slice(0, 5).map((f, i) => {
          ...
          const day = i === 0 ? 'Aaj' : d.toLocaleDateString('hi-IN', { weekday: 'short' });
          return (
